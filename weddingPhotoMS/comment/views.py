...
def addComment(request, id):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            #     print(i)
            if bv1 is not None:
                bv1_comment = Comment_Bv(c_bvId=order.order_bv.all()[0], c_customerId=user, c_content=bv1)
                bv2_comment = Comment_Bv(c_bvId=order.order_bv.all()[1], c_customerId=user, c_content=bv2)
                bv3_comment = Comment_Bv(c_bvId=order.order_bv.all()[2], c_customerId=user, c_content=bv3)
                bv4_comment = Comment_Bv(c_bvId=order.order_bv.all()[3], c_customerId=user, c_content=bv4)
                bv1_comment.save()
                bv2_comment.save()
                bv3_comment.save()
                bv4_comment.save()
            if bvs is not None:
                bvs_comment = Comment_Bvs(c_bvs=order.order_bvs, c_customer=user, c_content=bvs)
                bvs_comment.save()
            space_comment = Comment_Space(c_spaceId=order.order_space, c_customerId=user, c_content=space)
            space_comment.save()
            cameraman_comment = Comment_Camerman(c_cameraman=order.order_cameraman, c_customer=user, c_content=cameraman)
